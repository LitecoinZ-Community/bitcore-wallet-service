...
Insight.prototype.getUtxos = function(addresses, cb) {
  var url = this.url + this.apiPrefix + '/addrs/';
  var args = {
    method: 'POST',
    path: this.apiPrefix + '/addrs/utxo',
    json: {
      addrs: _.uniqBy([].concat(addresses)).join(',')
    },
  };
...
Insight.prototype.getTransactions = function(addresses, from, to, cb) {
  ...
  this._doRequest(args, function(err, res, txs) {
    ...
    // NOTE: Whenever Insight breaks communication with litecoinzd, it returns invalid data but no error code.
