...
WalletService.getInstance = function(opts) {
  ...
  if (version && version.agent == 'bwc') {
    if (version.major == 0 || (version.major == 0 && version.minor < 2)) {
      throw new ClientError(Errors.codes.UPGRADE_NEEDED, 'BWC clients < 0.2 are no longer supported.');
    }
  }
 ...
 * @param {string} opts.coin[='ltz'] - The coin for this wallet (ltz, bch).
 ...
 * @param {string} opts.coin[='ltz'] - The expected coin for this wallet (ltz, bch).
 ...
 * @param {Object} opts
 * @param {String} [opts.coin='ltz'] (optional)
...
WalletService.prototype.getFeeLevels = function(opts, cb) {
  ...
  function samplePoints() {
    var definedPoints = _.uniqBy(_.map(feeLevels, 'nbBlocks'));
    return _.uniqBy(_.flatten(_.map(definedPoints, function(p) {
