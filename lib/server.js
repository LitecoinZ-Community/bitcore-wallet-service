...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      self.logi('Total value in all utxos (' + Utils.formatAmount(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmount(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      self.logi('Value after fees in all utxos (' + Utils.formatAmount(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmount(txpAmount) + ')');
    ...
    self.logi('Big input threshold ' + Utils.formatAmount(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      //log.debug('The input contributes ' + Utils.formatAmount(netInputAmount));
      ...
      //log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmount(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          //log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmount((selected.length - 1) * feePerInput));
      ...
      //log.debug('Cumuled total so far: ' + Utils.formatAmount(total) + ', Net total so far: ' + Utils.formatAmount(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        //log.debug('Tx change: ', Utils.formatAmount(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bitcore_[txp.coin].Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          //log.debug('Change below dust threshold (' + Utils.formatAmount(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      //log.debug('Could not reach Txp total (' + Utils.formatAmount(txpAmount) + '), still missing: ' + Utils.formatAmount(txpAmount - netTotal));
  ...
  //log.debug('Selecting inputs for a ' + Utils.formatAmount(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        //log.debug('Fee for this selection: ' + Utils.formatAmount(fee));
      ...
      if (!err) {
        ...
        self.logi('Successfully built transaction. Total fees: ' + Utils.formatAmount(txp.fee) + ', total change: ' + Utils.formatAmount(change));
      } else {
