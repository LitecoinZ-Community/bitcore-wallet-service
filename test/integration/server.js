...
describe('Wallet service', function() {
    ...
    describe('shared wallets (BIP45)', function() {
      ...
      it('should create many addresses on simultaneous requests', function(done) {
        ...
        async.mapSeries(_.range(N), function(i, cb) {
          ...
          _.uniqBy(_.map(addresses, 'address')).length.should.equal(N);
          done();
        });
      });
    });
    ...
    it('should get balance', function(done) {
      helpers.stubUtxos(server, wallet, [1, 'u2', 3], function() {
        server.getBalance({}, function(err, balance) {
          ...
          server.getMainAddresses({}, function(err, addresses) {
            should.not.exist(err);
            var addresses = _.uniqBy(_.map(addresses, 'address'));
  ...
  var addrMap = {
    ltz: 't1h8SqgtM3QM5e2M8EzhhT1yL2PXXtA6oqe',
  }
  ...
  describe('#getNotifications', function() {
    ...
    it('should pull all notifications', function(done) {
      server.getNotifications({}, function(err, notifications) {
        ...
        var walletIds = _.uniqBy(_.map(notifications, 'walletId'));
        walletIds.length.should.equal(1);
        walletIds[0].should.equal(wallet.id);
        var creators = _.uniqBy(_.compact(_.map(notifications, 'creatorId')));
